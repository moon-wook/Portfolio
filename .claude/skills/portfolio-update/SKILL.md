...
분석 대상 프로젝트가 다른 서버에 있을 경우, 포트폴리오 루트의 `.env`에서 SSH 자격증명을 읽어 접속한다.

| 서버 구분 | `.env` 키 | 용도 |
|---|---|---|
| **개발 서버** | `Dev_ssh_host`, `Dev_user`, `Dev_password` | 개발 중인 프로젝트 / 실험 환경 분석 |
| **운영 서버** | `AI_ssh_host`, `AI_user`, `AI_password` | 배포된 운영 서비스 / 라이브 모델 조회 |

분석할 프로젝트가 **개발 단계**면 `Dev_*`, **운영 중**이면 `AI_*` 자격증명을 사용. 불명확할 때만 사용자에게 질문.
